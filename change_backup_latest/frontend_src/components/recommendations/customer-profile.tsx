...
const segmentColors: Record<string, string> = {
  Champions: "bg-yellow-500/15 text-yellow-400 border-yellow-500/30",
  Loyal: "bg-blue-500/15 text-blue-400 border-blue-500/30",
  Promising: "bg-green-500/15 text-green-400 border-green-500/30",
  "Potential Loyalist": "bg-cyan-500/15 text-cyan-400 border-cyan-500/30",
  "At Risk": "bg-orange-500/15 text-orange-400 border-orange-500/30",
  Hibernating: "bg-purple-500/15 text-purple-400 border-purple-500/30",
      ...
      <CardContent className="pt-5 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
          {[
            ...
            <div
              key={s.label}
              className="rounded-lg border bg-card/50 p-2.5 text-center"
            >
          ...
          <div>
            ...
            {profile.top_categories.length > 0 ? (
              profile.top_categories.map((c) => (
                <div
                  key={c.name}
                  className="flex justify-between py-1 border-b border-border/30 text-[0.76rem]"
                >
          ...
          <div>
            ...
            {profile.recent_orders.length > 0 ? (
              profile.recent_orders.map((o) => (
                <div
                  key={o.order_id}
                  className="py-1 border-b border-border/30 text-[0.72rem] text-muted-foreground"
                >
